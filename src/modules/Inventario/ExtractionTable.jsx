...
// Tabla y tarjetas editables de productos extraídos por la IA con matching semántico
...
export default function ExtractionTable({ items, onItemsChange, onConfirm, isConfirming }) {
  ...
  return (
    <div className="extraction-table-wrapper">
      ...
      <div className="extraction-desktop-container">
      ...
      {/* VISTA MÓVIL: Tarjetas verticales nativas */}
      <div className="extraction-mobile-cards">
        {items.map(item => (
          <div key={item._id} className={`extraction-mobile-card ${item.is_new ? 'is-new' : ''}`}>
            <div className="mobile-card-header">
              <div className="raw-name-cell">
                ...
                {item.is_new ? (
                  ...
                  <span className="match-warning" title={`Similitud: ${Math.round(item.match_score * 100)}%`}>
                    <AlertCircle size={12} />
                  </span>
                )}
              </div>
            ...
            <div className="mobile-card-field">
              <label className="mobile-label">Producto en catálogo</label>
              {item.is_new ? (
                <input
                  className="input input-sm"
                  value={item.mapped_name}
              ...
              <button
                ...
                onClick={() => toggleNew(item._id)}
              >
                {item.is_new ? '← Asociar a producto existente' : '+ Crear como producto nuevo'}
              </button>
            </div>

            <div className="mobile-card-inputs-grid">
              <div className="mobile-input-group">
                <label className="mobile-label">Cant.</label>
                <input
                  type="number"
                  className="input input-sm"
                  value={item.quantity}
              ...
              <div className="mobile-input-group">
                <label className="mobile-label">Costo u.</label>
                <div className="price-input-wrap">
                  <span>$</span>
                  <input
                    type="number"
              ...
              <div className="mobile-input-group">
                <label className="mobile-label">P. Venta</label>
                <div className="price-input-wrap">
                  <span>$</span>
                  <input
                    type="number"
